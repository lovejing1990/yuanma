		...
		public static void InsertOrUpdate(Character character)
		{
			using (var con = new MySqlConnection(Settings.Default.DataBaseConnectionString))
			{
				try
				{
					...
					command = new MySqlCommand("INSERT INTO character_records(characterid, accountid, worldid, type, type, charname, charrace, chargender, guid, v)" +
											   " VALUES (@characterid, @accountid, @worldid, @type, @type, @charname, @charrace, @chargender, @guid, @v)" +
					                           " ON DUPLICATE KEY UPDATE characterid = @characterid, accountid = @accountid, worldid = @worldid, type = @type," +
					                           " charname = @charname, charrace = @charrace, chargender = @chargender, guid = @guid, v = @v"
						, con);
