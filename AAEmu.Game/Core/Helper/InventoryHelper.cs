...
namespace AAEmu.Game.Core.Helper
{
    public class InventoryHelper
    {
        ...
        public static bool RemoveItemAndUpdateClient(Character character, Item item, int count)
        {
            ...
            {
                item.Count -= count;
                character.SendPacket(new SCItemTaskSuccessPacket(ItemTaskType.Destroy, new List<ItemTask> { new ItemCountUpdate(item, -count) }, new List<ulong>()));
                return true;
            }
            else
            {
                if (item.Count == count)
                {
                    ...
                    character.SendPacket(new SCItemTaskSuccessPacket(ItemTaskType.Destroy, new List<ItemTask> { new ItemRemove(item) }, new List<ulong>()));
                    return true;
                }
                else
                {
