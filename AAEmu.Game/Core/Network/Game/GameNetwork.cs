...
namespace AAEmu.Game.Core.Network.Game
{
    public class GameNetwork : Singleton<GameNetwork>
    {
        ...
        private GameNetwork()
        {
            ...
            // World
            // All opcodes here are updated for version client_3.0.3.0
            // Level 1
            RegisterPacket(0x000, 1, typeof(X2EnterWorldPacket));
            RegisterPacket(0x15D, 1, typeof(CSHgResponsePacket));
            RegisterPacket(0x163, 1, typeof(CSRSAEncryptAESkeyXORkeyPacket));
            RegisterPacket(0x186, 1, typeof(CSUnknownPacket_0x186)); //TODO: что за пакет?
            // Level 5
            //RegisterPacket(0x0C9, 5, typeof(CSListCharacterPacket)); //TODO opcode?
            RegisterPacket(0x22, 5, typeof(CSSecurityReportPacket));
            RegisterPacket(0x37, 5, typeof(CSPremiumServieceMsgPacket));
            RegisterPacket(0x72, 5, typeof(CSRestrictCheckPacket));
            RegisterPacket(0x0C9, 5, typeof(CSRefreshInCharacterListPacket));
            RegisterPacket(0x0EB, 5, typeof(CSLeaveWorldPacket));
            RegisterPacket(0x179, 5, typeof(CSCreateCharacterPacket));

            RegisterPacket(0x0E9, 5, typeof(CSSelectCharacterPacket));
            RegisterPacket(0x98, 5, typeof(CSSetLpManageCharacterPacket));

            RegisterPacket(0x3F, 5, typeof(CSSpawnCharacterPacket));

            // =====================================================================================
            //// All opcodes here are updated for version client_1.2
