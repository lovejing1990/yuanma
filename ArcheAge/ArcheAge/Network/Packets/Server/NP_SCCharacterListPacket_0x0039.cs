...
namespace ArcheAgeGame.ArcheAge.Network.Packets.Server
{
    public sealed class NP_CharacterListPacket_0x0039 : NetPacket
    {
		...
		public NP_CharacterListPacket_0x0039(ClientConnection net, int num, int last) : base(01, 0x0039)
		{
			...
			CharacterHolder.LoadClothsData(ref chr, chr.NewbieClothPackId); //дополнительно прочитать Head,Chest,Legs,Gloves,Feet из таблицы equip_pack_cloths
			CharacterHolder.LoadWeaponsData(ref chr, chr.NewbieWeaponPackId); //дополнительно прочитать Weapon,WeaponExtra,WeaponRanged,Instrument из таблицы equip_pack_weapons
			CharacterHolder.LoadCharacterBodyCoord(ref chr, chr.CharRace, chr.CharGender); //дополнительно прочитать body, x, y, z из таблицы charactermodel
			CharacterHolder.LoadZoneFaction(ref chr, chr.CharRace, chr.CharGender); //дополнительно прочитать FactionId,StartingZoneId из таблицы characters
