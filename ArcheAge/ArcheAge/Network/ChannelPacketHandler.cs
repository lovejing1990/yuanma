...
namespace ArcheAgeGame.ArcheAge.Network
{
	public class ChannelPacketHandler : PacketHandler<ChannelConnection>
	{
		...
		public void CZ_CONNECT(ChannelConnection conn, Packet packet)
		{
			...
			// Get account
			conn.Account = ArcheAgeGame.Instance.Database.GetAccount(accountName);
			...
			// Get character
			var character = ArcheAgeGame.Instance.Database.GetCharacter(conn.Account.Id, characterId);
			if (character == null)
			{
			...
			// Get map
			var map = ArcheAgeGame.Instance.World.GetMap(character.MapId);
			if (map == null)
			{
		...
		public void CZ_CHAT(ChannelConnection conn, Packet packet)
		{
			...
			if (!ArcheAgeGame.Instance.GmCommands.Process(conn, character, msg))
			{
		...
		public void CZ_CLICK_TRIGGER(ChannelConnection conn, Packet packet)
		{
			...
			ArcheAgeGame.Instance.ScriptManager.Call(conn, monster.DialogName);
		}
		...
		public void CZ_DIALOG_SELECT(ChannelConnection conn, Packet packet)
		{
			...
			ArcheAgeGame.Instance.ScriptManager.Resume(conn, option);
		}
		...
		public void CZ_DIALOG_ACK(ChannelConnection conn, Packet packet)
		{
			...
			if (type == 1)
			{
				ArcheAgeGame.Instance.ScriptManager.Resume(conn);
			}
			else
			{
		...
		public void CZ_DIALOG_STRINGINPUT(ChannelConnection conn, Packet packet)
		{
			...
			ArcheAgeGame.Instance.ScriptManager.Resume(conn, input);
		}
		...
		public void CZ_REQ_NORMAL_TX_NUMARG(ChannelConnection conn, Packet packet)
		{
			...
			else if (txType == TxType.Skills)
			{
				...
				// Check skill data
				var skillTreeData = ArcheAgeGame.Instance.Data.SkillTreeDb.FindSkills(job.Id, job.Circle);
		...
		public void CZ_ITEM_BUY(ChannelConnection conn, Packet packet)
		{
			...
			// Get shop
			var shopData = ArcheAgeGame.Instance.Data.ShopDb.Find(conn.ScriptState.CurrentShop);
			if (shopData == null)
			{
			...
			foreach (var purchase in purchases)
			{
				...
				// Get item
				var itemData = ArcheAgeGame.Instance.Data.ItemDb.Find(productData.ItemId);
				if (itemData == null)
				{
		...
		public void CZ_MAP_REVEAL_INFO(ChannelConnection conn, Packet packet)
		{
			...
			var mapData = ArcheAgeGame.Instance.Data.MapDb.Find(mapId);
			if (mapData == null)
			{
		...
		public void CZ_MAP_SEARCH_INFO(ChannelConnection conn, Packet packet)
		{
			...
			var mapData = ArcheAgeGame.Instance.Data.MapDb.Find(map);
			if (mapData == null)
			{
