...
namespace ArcheAgeGame.ArcheAge.World
{
	public class Map
	{
		...
		public Map(int id, string name)
		{
			...
			this._monsters = new Dictionary<int, Monster>();
		...
		private void Disappearances()
		{
			...
			lock (this._monsters)
				toDisappear = this._monsters.Values.Where(a => a.DisappearTime < now).ToList();
		...
		{
			lock (this._characters)
				this._characters.Remove(character.Handle);
		...
		public Character GetCharacter(int handle)
		{
			Character result;
			lock (this._characters)
				this._characters.TryGetValue(handle, out result);
			return result;
		}
		...
		public void AddMonster(Monster monster)
		{
			...
			lock (this._monsters)
				this._monsters[monster.Handle] = monster;
		}
		...
		{
			lock (this._monsters)
				this._monsters.Remove(monster.Handle);
		...
		public Monster GetMonster(int handle)
		{
			Monster result;
			lock (this._monsters)
				this._monsters.TryGetValue(handle, out result);
			return result;
		}
		...
		{
			lock (this._characters)
				return this._characters.Values.Where(a => a != character && character.Position.InRange2D(a.Position, VisibleRange)).ToArray();
		}
		...
		{
			lock (this._monsters)
				return this._monsters.Values.ToArray();
		}
		...
		{
			lock (this._monsters)
				return this._monsters.Values.Where(a => character.Position.InRange2D(a.Position, VisibleRange)).ToArray();
		}
		...
		public void RemoveScriptedEntities()
		{
			...
			lock (this._monsters)
				toRemove.AddRange(this._monsters.Values.Where(a => a.NpcType == NpcType.NPC));
		...
		public Monster GetNearbyWarp(Position pos)
		{
			...
			lock (this._monsters)
				return this._monsters.Values.FirstOrDefault(a => a.IsWarp && a.Position.InRange2D(pos, 35));
		}
		...
		public virtual void Broadcast(Packet packet, IEntity source, bool includeSource = true)
		{
			lock (this._characters)
			{
				foreach (var character in this._characters.Values.Where(a => (includeSource || a != source) && a.Position.InRange2D(source.Position, VisibleRange)))
					character.Connection.Send(packet);
			}
		}
	}
