...
namespace ArcheAgeGame.ArcheAge.World
{
	...
	public class Skills
	{
		...
		/// </summary>
		public int Count { get { lock (this._skills) return this._skills.Count; } }
		...
		{
			lock (this._skills)
				return this._skills.Remove(skillId);
		}
		...
		public Skill Get(int skillId)
		{
			lock (this._skills)
			{
				this._skills.TryGetValue(skillId, out var result);
				return result;
			}
		}
		...
		{
			lock (this._skills)
				return this._skills.Values.FirstOrDefault(a => a.Data.ClassName == skillClassName);
		}
		...
		{
			lock (this._skills)
				return this._skills.Values.ToArray();
		}
		...
		public int GetMaxLevel(int skillId)
		{
			...
			{
				var skillTreeDataList = ArcheAgeGame.Instance.Data.SkillTreeDb.FindSkills(job.Id, job.Circle).Where(a => a.SkillId == skillId);
