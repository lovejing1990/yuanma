...
namespace ArcheAgeGame.ArcheAge.World
{
	public class Character : IEntity, ICommander, IPropertyObject
	{
		...
		/// </summary>
		public Direction2 HeadDirection { get; set; }
		...
		{
			get { return this._statByLevel; }
			set { this._statByLevel = Math2.Clamp(0, short.MaxValue, value); }
		}
		...
		{
			get { return this._statByBonus; }
			set { this._statByBonus = Math2.Clamp(0, short.MaxValue, value); }
		}
		...
		public int UsedStat
		{
			get { return this._usedStat; }
			set { this._usedStat = Math2.Clamp(0, this.StatByLevel + this.StatByBonus, value); }
		}
		...
		public int AbilityPoints
		{
			...
			set { this._abilityPoints = Math2.Clamp(0, short.MaxValue, value); }
		}
		...
		public int ClassLevel
		{
			get
			{
				...
				for (var i = 1; i < max; ++i)
				{
					var needed = ArcheAgeGame.Instance.Data.ExpDb.GetNextTotalClassExp(rank, i);
					if (totalExp < needed)
					{
						return i;
					}
		...
		{
			get { return this._sp; }
			set { this._sp = Math2.Clamp(0, this.MaxSp, value); }
		}
		private int _sp;
		...
		public int Stamina
		{
			...
			set { this._stamina = Math2.Clamp(0, this.MaxStamina, value); }
		}
		...
		{
			this.Direction = new Direction2(x, y);
		}
		...
		{
			this.HeadDirection = new Direction2(x, y);
		}
		...
		{
			var map = ArcheAgeGame.Instance.Data.MapDb.Find(mapName);
			if (map == null)
			{
		...
		{
			var map = ArcheAgeGame.Instance.World.GetMap(mapId);
			if (map == null)
			{
		...
		public void FinalizeWarp()
		{
			...
			ArcheAgeGame.Instance.Database.SaveCharacter(this);

			// Get channel
			var channelId = 1;
			var archeAgeGame = ArcheAgeGame.Instance.Data.ServerDb.FindChannel(channelId);
			if (archeAgeGame == null)
			{
				Log.Error("Channel with id '{0}' not found.", channelId);
				return;
			}

			Send.ZC_MOVE_ZONE_OK(this.Connection, archeAgeGame.Ip, archeAgeGame.Port, this.MapId);
		}
		...
		public void LevelUp(int amount = 1)
		{
			...
			this.MaxExp = ArcheAgeGame.Instance.Data.ExpDb.GetNextExp(this.Level);
		...
		public void GiveExp(int exp, int classExp, Monster monster)
		{
			...
			var rank = this.Jobs.GetCurrentRank();
			var job = this.Job;
			var maxTotalExp = ArcheAgeGame.Instance.Data.ExpDb.GetNextTotalClassExp(rank, 15);
		...
		public void LookAround()
		{
			...
			lock (this._lookAroundLock)
			{
				...
				// Appears
				var appearMonsters = currentlyVisibleMonsters.Except(this._visibleMonsters);
				var appearCharacters = currentlyVisibleCharacters.Except(this._visibleCharacters);

				// Disappears
				var disappearMonsters = this._visibleMonsters.Except(currentlyVisibleMonsters);
				var disappearCharacters = this._visibleCharacters.Except(currentlyVisibleCharacters);
				...
				this._visibleMonsters = currentlyVisibleMonsters;
				this._visibleCharacters = currentlyVisibleCharacters;
			}
		}
		...
		public void CloseEyes()
		{
			...
			lock (this._lookAroundLock)
			{
				foreach (var monster in this._visibleMonsters)
				{
					Send.ZC_LEAVE(this.Connection, monster);
				}

				foreach (var character in this._visibleCharacters)
				{
					Send.ZC_LEAVE(this.Connection, character);
				}

				this._visibleMonsters = new Monster[0];
				this._visibleCharacters = new Character[0];
			}
		}
		...
		public int UpdateStance()
		{
			...
			this.Stance = ArcheAgeGame.Instance.Data.StanceConditionDb.FindStanceId(jobId, riding, rightHand, leftHand);
