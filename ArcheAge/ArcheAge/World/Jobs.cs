...
namespace ArcheAgeGame.ArcheAge.World
{
	...
	public class Jobs
	{
		...
		/// </summary>
		public int Count { get { lock (this._jobs) return this._jobs.Count; } }
		...
		{
			lock (this._jobs)
				return this._jobs.Remove(jobId);
		}
		...
		public Job Get(JobId jobId)
		{
			lock (this._jobs)
			{
				this._jobs.TryGetValue(jobId, out var job);
				return job;
			}
		}
		...
		public int GetCurrentRank()
		{
			var rank = 0;
			lock (this._jobs)
				rank = this._jobs.Values.Sum(a => (int)a.Circle);
