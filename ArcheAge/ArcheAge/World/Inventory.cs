...
namespace ArcheAgeGame.ArcheAge.World
{
	public class Inventory
	{
		...
		{
			this._items = new Dictionary<InventoryCategory, List<Item>>();
			foreach (InventoryCategory category in Enum.GetValues(typeof(InventoryCategory)))
				this._items.Add(category, new List<Item>());

			this._equip = new Dictionary<EquipSlot, Item>(Items.EquipSlotCount);
			for (EquipSlot slot = 0; slot < (EquipSlot)Items.EquipSlotCount; ++slot)
				this._equip.Add(slot, new DummyEquipItem(slot));

			this._itemsWorldIndex = new Dictionary<long, Item>();
		...
		public int[] GetEquipIds()
		{
			...
			lock (this._syncLock)
				return this._equip.Where(a => (int)a.Key <= Items.EquipSlotCount).OrderBy(a => a.Key).Select(a => a.Value.Id).ToArray();
		}
		...
		{
			lock (this._syncLock)
				return this._equip.ToDictionary(a => a.Key, a => a.Value);
		}
		...
		public IDictionary<int, Item> GetItems()
		{
			...
			lock (this._syncLock)
			{
				foreach (var category in this._items)
				{
		...
		public IDictionary<int, long> GetIndices(InventoryCategory category)
		{
			...
			lock (this._syncLock)
			{
				...
				var items = this._items[category];
		...
		{
			lock (this._syncLock)
				return this._items.SelectMany(a => a.Value).Any(a => a.Id == itemId);
		}
		...
		{
			lock (this._syncLock)
				return this._items.SelectMany(a => a.Value).Where(a => a.Id == itemId).Sum(a => a.Amount);
		}
		...
		public Item GetItem(long worldId)
		{
			Item item;
			lock (this._syncLock)
				this._itemsWorldIndex.TryGetValue(worldId, out item);
		...
		public Item GetItem(EquipSlot slot)
		{
			Item item;
			lock (this._syncLock)
				this._equip.TryGetValue(slot, out item);
		...
		public void Add(Item item, InventoryAddType addType)
		{
			...
			Send.ZC_OBJECT_PROPERTY(this._character, PropertyId.PC.NowWeight);
		}
		...
		private int FillStacks(Item item, InventoryAddType addType, bool silent)
		{
			...
			foreach (var index in stacks)
			{
				lock (this._syncLock)
				{
					var categoryItem = this._items[cat][index];
					...
					if (!silent)
					{
						...
						Send.ZC_ITEM_ADD(this._character, categoryItem, categoryIndex, add, adjustedAddType);
					}
				}
		...
		private void AddStack(Item item, InventoryAddType addType, bool silent)
		{
			...
			lock (this._syncLock)
			{
				this._items[cat].Add(item);
				this._itemsWorldIndex[item.ObjectId] = item;

				if (!silent)
				{
					var categoryIndex = cat.GetIndex(this._items[cat].Count - 1);
					Send.ZC_ITEM_ADD(this._character, item, categoryIndex, item.Amount, addType);
				}
			}
		}
		...
		private List<int> GetStacks(InventoryCategory cat, int itemId)
		{
			...
			lock (this._syncLock)
			{
				var categoryItems = this._items[cat];
		...
		{
			lock (this._syncLock)
				this._equip[slot] = item;

			this._character.UpdateStance();
		}
		...
		public InventoryResult Equip(EquipSlot slot, long worldId)
		{
			...
			var collision = false;
			lock (this._syncLock)
				collision = !(this._equip[slot] is DummyEquipItem);
			...
			{
				this._equip[slot] = item;
				this._items[item.Data.Category].Remove(item);
				this._itemsWorldIndex.Remove(item.ObjectId);
			}
			...
			// Update client
			Send.ZC_ITEM_REMOVE(this._character, item.ObjectId, 1, InventoryItemRemoveMsg.Equipped, InventoryType.Inventory);
			Send.ZC_ITEM_EQUIP_LIST(this._character);
			//Send.ZC_ITEM_INVENTORY_INDEX_LIST(_character, item.Data.Category);
			Send.ZC_UPDATED_PCAPPEARANCE(this._character);
		...
		public InventoryResult Unequip(EquipSlot slot)
		{
			...
			lock (this._syncLock)
				this._equip[slot] = new DummyEquipItem(slot);
		...
		private InventoryResult Remove(Item item)
		{
			lock (this._syncLock)
			{
				if (!this._items[item.Data.Category].Remove(item))
					return InventoryResult.ItemNotFound;

				this._itemsWorldIndex.Remove(item.ObjectId);
			}

			Send.ZC_ITEM_REMOVE(this._character, item.ObjectId, item.Amount, InventoryItemRemoveMsg.Destroyed, InventoryType.Inventory);
			//Send.ZC_ITEM_INVENTORY_INDEX_LIST(_character, item.Data.Category);
			Send.ZC_OBJECT_PROPERTY(this._character, PropertyId.PC.NowWeight);
		...
		public InventoryResult Remove(Item item, int amount, InventoryItemRemoveMsg msg)
		{
			...
			lock (this._syncLock)
			{
				if (!this._items[item.Data.Category].Contains(item))
					return InventoryResult.ItemNotFound;
			}
			...
			else
			{
				...
				Send.ZC_ITEM_REMOVE(this._character, item.ObjectId, amount, msg, InventoryType.Inventory);
				Send.ZC_OBJECT_PROPERTY(this._character, PropertyId.PC.NowWeight);
			}
		...
		public int Remove(int itemId, int amount, InventoryItemRemoveMsg msg)
		{
			...
			foreach (var itemkv in this.GetItems(a => a.Id == itemId))
			{
				...
				if (reduce == itemAmount)
				{
					lock (this._syncLock)
					{
						this._items[category].Remove(item);
						this._itemsWorldIndex.Remove(item.ObjectId);

					}
				}

				Send.ZC_ITEM_REMOVE(this._character, item.ObjectId, reduce, msg, InventoryType.Inventory);
				//Send.ZC_ITEM_INVENTORY_INDEX_LIST(_character, item.Data.Category);
			}
		...
		public void Sort(InventoryOrder order)
		{
			lock (this._syncLock)
			{
				...
				foreach (var category in this._items.Keys)
				{
					switch (order)
					{
						case InventoryOrder.Price:
							items[category] = this._items[category].OrderBy(a => a.Data.Price).ToList();
							break;

						case InventoryOrder.Weight:
							items[category] = this._items[category].OrderBy(a => a.Data.Weight).ToList();
							break;
						...
						case InventoryOrder.Name:
							items[category] = this._items[category].OrderBy(a => a.Data.Name).ToList();
							break;

						default:
							items[category] = this._items[category].OrderBy(a => a.Id).ToList();
							break;
					}
				}
			...
			Send.ZC_ITEM_INVENTORY_LIST(this._character);
		}
		...
		public float GetNowWeight()
		{
			...
			lock (this._syncLock)
			{
				result += this._items.SelectMany(a => a.Value).Sum(a => a.Data.Weight);
				result += this._equip.Values.Where(a => !(a is DummyEquipItem)).Sum(a => a.Data.Weight);
			}
		...
		public void Debug()
		{
			...
			Log.Debug("Equip");
			foreach (var item in this._equip.ToDictionary(a => a.Key, a => a.Value))
		...
		public InventoryResult Clear()
		{
			...
			foreach (var category in modifiedCategories)
				Send.ZC_ITEM_INVENTORY_INDEX_LIST(this._character, category);
