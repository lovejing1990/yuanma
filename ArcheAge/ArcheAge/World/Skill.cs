...
namespace ArcheAgeGame.ArcheAge.World
{
	public class Skill : IPropertyObject
	{
		...
		/// something.
		/// </remarks>
		public long ObjectId { get; } = ArcheAgeGame.Instance.World.CreateSkillObjectId();
		...
		public Skill(Character character, int skillId, int level)
		{
			...
			this.Id = skillId;
			this.Level = level;
			this.Data = ArcheAgeGame.Instance.Data.SkillDb.Find(skillId) ?? throw new ArgumentException($"Unknown skill '{skillId}'.");
