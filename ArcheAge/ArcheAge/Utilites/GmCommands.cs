...
namespace ArcheAgeGame.ArcheAge.Utilites
{
	...
	public partial class GmCommands : CommandManager<GmCommand, GmCommandFunc>
	{
		...
		public bool Process(ChannelConnection conn, Character character, string message)
		{
			...
			if (args.Length == 0 || (!args[0].StartsWith("/") && !args[0].StartsWith(ArcheAgeGame.Instance.Conf.Commands.Prefix) && !args[0].StartsWith(ArcheAgeGame.Instance.Conf.Commands.Prefix2)))
			{
				return false;
			}

			// Get command name
			var isCharCommand = args[0].StartsWith(ArcheAgeGame.Instance.Conf.Commands.Prefix2);
			var commandName = args[0].TrimStart('/', ArcheAgeGame.Instance.Conf.Commands.Prefix[0]);
			...
			if (isCharCommand)
			{
				...
				target = ArcheAgeGame.Instance.World.GetCharacterByTeamName(targetName);
				if (target == null)
				{
			...
			var auth = ArcheAgeGame.Instance.Conf.Commands.GetAuth(commandName);
			...
			{
				character.ServerMessage("Invalid argument, usage: {0}{1} {2}", ArcheAgeGame.Instance.Conf.Commands.Prefix, commandName, command.Usage);
			}
