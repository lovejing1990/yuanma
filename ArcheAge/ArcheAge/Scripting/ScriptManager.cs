...
namespace ArcheAgeGame.ArcheAge.Scripting
{
	public class ScriptManager
	{
		...
		{
			this._states = new Dictionary<IntPtr, ScriptState>();
		...
		{
			this.GL = Melua.luaL_newstate();

			Melua.melua_openlib(this.GL, new[]
			{
			...
			Melua.lua_atpanic(this.GL, Melua.CreateFunctionReference(this.GL, this.OnPanic));

			// Functions
			// --------------------------------------------------------------

			// Setup
			this.Register(this.addnpc);
			this.Register(this.addwarp);

			// General
			this.Register(this.var);
			this.Register(this.logdebug);
			this.Register(this.sqlquery);
			this.Register(this.sqlescape);
			this.Register(this.addonmsg);

			// Dialog
			this.Register(this.msg);
			this.Register(this.select);
			this.Register(this.close);
			this.Register(this.input);
			this.Register(this.numinput);
			this.Register(this.openshop);

			// Information
			this.Register(this.getpc);
			this.Register(this.getnpc);
			this.Register(this.gettime);

			// Jobs
			this.Register(this.isjob);
			this.Register(this.isjobclass);
			this.Register(this.addjob);
			this.Register(this.removejob);

			// Inventory
			this.Register(this.hasitem);
			this.Register(this.countitem);
			this.Register(this.additem);
			this.Register(this.removeitem);

			// Action
			this.Register(this.warp);
			this.Register(this.resetstats);
			this.Register(this.changehair);
			this.Register(this.spawn);
			this.Register(this.levelup);
		}
		...
		private void InitializeVariables()
		{
			...
			ArcheAgeGame.Instance.Database.LoadVars(GlobalVariableOwner, this.Variables.Perm);
			this._globalVarSaver = new Timer(this.SaveGlobalVars, null, VariableSaveInterval, VariableSaveInterval);
			this._lastVarChange = DateTime.Now;
		}
		...
		{
			var func = Melua.CreateFunctionReference(this.GL, function);
			Melua.lua_register(this.GL, function.Method.Name, func);
		}
		...
		public void Reload()
		{
			ArcheAgeGame.Instance.World.RemoveScriptedEntities();
			this.Load();
		}
		...
		private bool LoadFile(string filePath)
		{
			...
			// Load file
			var result = Melua.luaL_loadfile(this.GL, filePath);
			if (result != 0)
			{
				Log.Error("ScriptManager.LoadFile: Failed to compile '{0}' (Error code: {1}).\n{2}", filePath, result, Melua.lua_tostring(this.GL, -1));
				return false;
			}

			// Execute it
			if (Melua.lua_pcall(this.GL, 0, 0, 0) != 0)
			{
				Log.Error("ScriptManager.LoadFile: Failed to load '{0}'.\n{1}", filePath, Melua.lua_tostring(this.GL, -1));
				return false;
			}
		...
		public LuaThread GetNewThread(ScriptState state)
		{
			...
			IntPtr NL;
			int index;
			lock (this._glSyncLock)
			{
				// Create new thread and save index, so it can be removed later.
				NL = Melua.lua_newthread(this.GL);
				index = Melua.lua_gettop(this.GL);
			}

			lock (this._states)
			{
				this._states.Add(NL, state);
			}
		...
		public void RemoveThread(LuaThread thread)
		{
			...
			lock (this._glSyncLock)
			{
				...
				Melua.lua_remove(this.GL, thread.StackIndex);
		...
		{
			return ArcheAgeGame.Instance.Data.DialogDb.Exists(value);
		}
		...
		public void Call(ChannelConnection conn, string functionName)
		{
			...
			var top = Melua.lua_gettop(this.GL);
		...
		public void Resume(ChannelConnection conn, params object[] arguments)
		{
			...
			if (argc != 0)
			{
				foreach (var arg in arguments)
				{
					if (arg is byte)
					{
						Melua.lua_pushinteger(NL, (byte)arg);
					}
					else if (arg is short)
					{
						Melua.lua_pushinteger(NL, (short)arg);
					}
					else if (arg is int)
					{
						Melua.lua_pushinteger(NL, (int)arg);
					}
					else if (arg is string)
					{
						Melua.lua_pushstring(NL, (string)arg);
					}
					else
					{
		...
		private ChannelConnection GetConnectionFromState(IntPtr L)
		{
			...
			lock (this._states)
			{
				if (!this._states.TryGetValue(L, out state))
				{
		...
		{
			if (this.Variables.Perm.LastChange <= this._lastVarChange)
			{
				return;
			}

			this._lastVarChange = this.Variables.Perm.LastChange;

			try
			{
				ArcheAgeGame.Instance.Database.SaveVariables(GlobalVariableOwner, this.Variables.Perm);
		...
		private int addnpc(IntPtr L)
		{
			...
			var map = ArcheAgeGame.Instance.World.GetMap(mapName);
			if (map == null)
			{
			...
			monster.Direction = new Direction2(direction);
		...
		private int addwarp(IntPtr L)
		{
			...
			// Check "from" map data
			var fromMapData = ArcheAgeGame.Instance.Data.MapDb.Find(fromMapName);
			if (fromMapData == null)
			{
			...
			// Check map in world
			var map = ArcheAgeGame.Instance.World.GetMap(fromMapData.Id);
			if (map == null)
			{
			...
			// Check "to" map data
			var toMapData = ArcheAgeGame.Instance.Data.MapDb.Find(toMapName);
			if (toMapData == null)
			{
		...
		private int openshop(IntPtr L)
		{
			...
			if (!ArcheAgeGame.Instance.Data.ShopDb.Exists(shopName))
			{
		...
		private int additem(IntPtr L)
		{
			...
			var itemData = ArcheAgeGame.Instance.Data.ItemDb.Find(itemId);
			if (itemData == null)
			{
		...
		private int sqlquery(IntPtr L)
		{
			...
			try
			{
				result = ArcheAgeGame.Instance.Database.GetQueryResult(sqlQuery, out affectedRows, out lastInsertId);
			}
			catch (Exception ex)
			{
