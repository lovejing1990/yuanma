...
namespace LocalCommons.Data.Database
{
	...
	public class ExpDb : DatabaseJson<object>
	{
		...
		public int GetNextExp(int level)
		{
			...
			if (level > this._exp.Count)
				return 0;

			var index = level - 1;
			var exp = this._exp[index];
		...
		public int GetNextTotalClassExp(int rank, int level)
		{
			...
			var data = this._classExp.Where(a => a.Rank == rank);
			if (!data.Any())
		...
		protected override void AfterLoad()
		{
			...
			this.Entries.AddRange(this._exp.Cast<object>());
			this.Entries.AddRange(this._classExp);
		}
	}
}
