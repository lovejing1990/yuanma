...
namespace LocalCommons.Utilities.Configuration
{
	/// <summary>
	...
	public class ConfFile
	{
		...
		private void LoadFile(string filePath)
		{
			using (var fr = new FileReader(filePath))
			{
				foreach (var line in fr)
				{
					...
					this._options[line.Value.Substring(0, pos).Trim()] = line.Value.Substring(pos + 1).Trim();
				}
			}
		}
		...
		{
			if (!this._options.TryGetValue(option, out var value))
				return defaultValue;
		...
		{
			return !this._options.TryGetValue(option, out var value) ? defaultValue : value;
		}
